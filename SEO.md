...
- A labeled advertising rail combines one Smartlink and one 300x250 Adsterra banner beside the game card on wide and medium screens; mobile places it immediately after the card and before the color legend. It shares the game-entry sightline but remains outside all game controls and result containers. The Smartlink opens in a new tab and uses `rel="sponsored noopener noreferrer"`. Popunder and Social Bar scripts are limited to the two homepages. None of these placements replaces crawlable copy; trust pages, supporting articles, and the 404 page remain ad-free.
...
- AdSense review decisions and official policy links are maintained in `ADSENSE_REVIEW.md`. The root `ads.txt` contains the seller record designated by the owner. The separate Adsterra Banner, Popunder, Social Bar, and Smartlink placements are recorded in `README.md` and `progress.md`.
