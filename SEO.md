...
- Google Search Console: `sc-domain:xn--80ahqbfrbqm.com` is DNS-verified; the production sitemap was read successfully on 2026-07-12 and reports four discovered pages.
- Production source audit on 2026-07-13: a normal browser user agent and Googlebot both received the same 33,716-byte Russian HTML document. Its SHA-1 matched the repository `index.html` exactly, and a raw-HTML text parser counted 987 Russian words without executing JavaScript. The English production response contained 1,298 words by the same parser; small differences from the table above come from tokenizer rules.
- Do not use `https://colordice.vercel.app/` to audit this repository. That reference site returns an empty `<div id="app"></div>` and stores its UI in a Base64 JavaScript payload; its raw response has only four visible words. A report describing roughly 22 words and suspected CSR is evidence that the reference URL or a visual-only start-screen view was measured, not `https://xn--80ahqbfrbqm.com/`.
