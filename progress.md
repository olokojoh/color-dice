...
- Deployment `234acdea-c625-47ff-8df2-22f5c5310621` was an `ad_hoc` production promotion: it reported branch `main` and commit `898e0df`, while remote GitHub `main` still pointed to `8438dc9`. Its clone and build stages were idle, unlike the complete `dev` preview build three minutes earlier. The corrective release fast-forwarded `main` through the two `dev` commits and this handoff update, then relied on a new GitHub `main` push for the production clone/build/deploy. Future production commits must be reachable from the remote `main` ref.
...
- The exposure audit found 13 historical Pages deployments whose immutable hostnames returned `README.md`; all 13 were removed through the Pages API. Only clean production `234acdea-c625-47ff-8df2-22f5c5310621` and clean preview `8e1d012d-fd07-407d-8e2b-25b86725593c` remain in the deployment list, and both return 404 for repository files. A removed immutable hostname still returned the old file with `Cache-Control: public, max-age=0, must-revalidate`, including with a query string. The account cannot purge the `pages.dev` zone; immediate removal of that residual hostname requires Cloudflare Support or deletion of the whole Pages project. The active custom-domain WAF rule prevents this residual Pages behavior from exposing files through production.
