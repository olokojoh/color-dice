...
- Cloudflare Pages project: `color-dice` (`color-dice-4uo.pages.dev`), source type `github`, repository `olokojoh/color-dice`, production branch `main`, automatic production and preview deployments enabled. A 2026-07-28 pre-release observation recorded no build command and output directory `/`. Releases containing the allowlisted build must use build command `node build-pages.mjs` and output directory `dist`; Pages must publish `dist`, not the repository root.
...
- The July 28 R03 repair sets all four sitemap `lastmod` values to 2026-07-27 and corrects stale repository-state claims. The local R06 and R09 worker approach was not deployed and is removed in R13. R13 uses `node build-pages.mjs` and `public-files.json` to build an allowlisted `dist/` without repository documents, build files, local output, or run evidence. Document noindex rules are no longer needed because those files are absent from the artifact. A 2026-07-28 pre-release observation recorded R03 and R13 as uncommitted and not deployed under the former root-output configuration. The release procedure must use build command `node build-pages.mjs`, publish output directory `dist`, and verify production afterward. Run artifacts live in `.hermes/runs/20260728-colordice-adsense-recheck-e0135fc/`.
...
- The AdSense and SEO changes reviewed through 2026-07-18 were merged into `main` and deployed; a July 27 probe found production serving commit `8438dc9`. The root `ads.txt` contains the owner-designated, non-placeholder publisher record. The July 27 Mode B edits are recorded on `dev` at `e0135fc`. Every release containing the R03 sitemap changes and R13 build-output repair requires peer review of the exact source, Pages build command `node build-pages.mjs`, output directory `dist`, deployment of that artifact, and production verification. Deployment remains an owner-authorized step.
