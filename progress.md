...
- `https://xn--80ahqbfrbqm.com/sitemap.xml` is submitted in Search Console. On 2026-07-20 the UI showed `Sitemap could not be read` with one `General HTTP error`, last read Jul 15, while direct Googlebot, IPv4, and IPv6 requests all returned HTTP 200 and valid XML. The main branch now provides explicit sitemap/robots response headers through `_headers`; resubmit after the Git-triggered Cloudflare Pages deployment.
