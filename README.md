...
A production check on 2026-07-18 found the deployment behind the workspace: `/en/privacy/` returned 404 and the live homepage still contained the removed start screen and player-code panel. Later pushes closed that gap. On 2026-07-27 a fresh read-only probe confirmed that `HEAD` and `origin/main` both point to `8438dc9` and that all 12 HTML routes, `robots.txt`, `sitemap.xml`, the seller record, and both Open Graph images return HTTPS 200 with bytes equal to that commit, while an unknown route returns 404. The July 27 Mode B implementation edits (rules-table color labels, the Article `dateModified`, the Russian preview alt text, one `/kak-igrat/` sentence, the English scratch overlay title, the regenerated English preview image, and these documentation updates) are uncommitted local changes and are not deployed. No commit, push, deployment, or AdSense account action was performed.
