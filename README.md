...
Production uses the Cloudflare Pages project `color-dice`, linked to the repository's `main` branch. Pages releases must run `node build-pages.mjs` and publish the `dist` output directory, not the repository root. A 2026-07-28 pre-release observation recorded the former configuration with no build command and `/` as the output directory. Every push to `main` triggers a production deployment; pull requests receive preview deployments. The generated Pages hostname is `https://color-dice-4uo.pages.dev`.
